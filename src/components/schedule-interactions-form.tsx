...
export function ScheduleInteractionsForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    toast.success(`Interacción de ${activeTab === "tiktok" ? "TikTok" : "Facebook"} programada correctamente`)
  }
      ...
      <TabsContent value="tiktok">
        <Card className="w-full border-t-0 rounded-tl-none rounded-tr-none">
          <CardHeader>
            ...
            <CardDescription>Configura las interacciones que deseas programar en TikTok</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="tiktok-url" placeholder="https://tiktok.com/@usuario/video/123456789" />
                </div>
                ...
                {addComment && (
                  <div className="space-y-2">
                    <Label htmlFor="tiktok-comment-text">Texto del Comentario</Label>
                    <Textarea id="tiktok-comment-text" placeholder="Escribe tu comentario aquí..." />
                  </div>
                )}

                <div className="space-y-2">
                  <Label htmlFor="tiktok-views">Cantidad de Vistas a Generar</Label>
                  <Input id="tiktok-views" type="number" min="0" placeholder="1000" />
                </div>

                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="tiktok-devices">Dispositivos a Utilizar</Label>
                    <Select defaultValue="all">
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="group1">Grupo 1 (5 dispositivos)</SelectItem>
                        <SelectItem value="group2">Grupo 2 (3 dispositivos)</SelectItem>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select defaultValue="now">
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="tomorrow">Mañana a las 10:00</SelectItem>
                        <SelectItem value="custom">Personalizado</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
          ...
          <CardFooter className="flex justify-end">
            <Button onClick={handleSubmit} className="bg-black hover:bg-black/80">
      ...
      <TabsContent value="facebook">
        <Card className="w-full border-t-0 rounded-tl-none rounded-tr-none">
          <CardHeader>
            ...
            <CardDescription>Configura las interacciones que deseas programar en Facebook</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="facebook-url" placeholder="https://facebook.com/usuario/posts/123456789" />
                </div>
                ...
                {addComment && (
                  <div className="space-y-2">
                    <Label htmlFor="facebook-comment-text">Texto del Comentario</Label>
                    <Textarea id="facebook-comment-text" placeholder="Escribe tu comentario aquí..." />
                  </div>
                )}
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      {showGroupList ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                    </Button>
                          ...
                          <ScrollArea className="h-60">
                            <div className="space-y-2 p-2">
                              {facebookGroups.map((group) => (
                                ...
                                >
                                  <Checkbox checked={selectedGroups.includes(group.id)} id={`group-${group.id}`} />
                                  <Label htmlFor={`group-${group.id}`} className="font-normal cursor-pointer flex-1">
                                    {group.name}
                                  </Label>
                                  {selectedGroups.includes(group.id) && <Check className="h-4 w-4 text-blue-600" />}
                                </div>
                              ))}
                            </div>
                          </ScrollArea>
                  ...
                  {selectedGroups.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {selectedGroups.map((groupId) => {
                        ...
                        return (
                          <Badge key={groupId} variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">
                            {group?.name}
                          </Badge>
                        )
                      })}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label htmlFor="facebook-devices">Dispositivos a Utilizar</Label>
                    <div className="relative">
                      <Button
                        type="button"
                        variant="outline"
                                ...
                                {connectedDevices.map((device) => (
                                  ...
                                  >
                                    ...
                                    <div className="flex-1">
                                      <Label htmlFor={`device-${device.id}`} className="font-normal cursor-pointer">
                                        {device.name}
                                      </Label>
                                      <p className="text-xs text-muted-foreground">{device.platform}</p>
                                    </div>
                                    {selectedDevices.includes(device.id) && <Check className="h-4 w-4 text-blue-600" />}
                                  </div>
                                ))}
                              </div>
                            </ScrollArea>
          ...
          <CardFooter className="flex justify-end">
            <Button onClick={handleSubmit} className="bg-[#1877F2] hover:bg-[#1877F2]/80">
              Programar Interacción
            </Button>
          </CardFooter>
        </Card>
